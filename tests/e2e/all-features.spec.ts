...
test.describe("fluxos completos do WillTreino local", () => {
  ...
  test("1. Dashboard: carrega saudação, treino de hoje, resumo semanal e atalhos", async ({ page }) => {
    ...
    await expect(page.getByRole("link", { name: /Começar treino|Continuar treino/i })).toBeVisible();
  });
  ...
  test("2. Onboarding: navega pelas etapas e preenche preferências", async ({ page }) => {
    ...
    await expect(page.getByRole("heading", { name: /Qual é o seu objetivo principal\?/i })).toBeVisible();
    
    await page.locator("label").filter({ hasText: /Ganhar massa muscular/i }).click();
    await page.getByRole("button", { name: "Próximo" }).click();
    
    await expect(page.getByRole("heading", { name: /Altura e peso/i })).toBeVisible();
  });

  test("3. Treino (/workout): carrega exercício ativo, controles de carga/reps e permite registrar série", async ({ page }) => {
